...
def residuals(p,y,x):
    ...
    else: raise Exception('Number of coefficients provided for residuals(p,y,x) is wrong!!!')
...
def perform_fitting(X,Y):
    ...
    #fit triple lorentz
    coefficients = [[3.5,521.6,12e3],[70,480,6000],[25,500,14000]]#[[3.5,521.6,12e3],[60,480,30],[30,496,3000]]  # [hwhm, peak center, intensity] for three peaks
    ...
    y_bg_peak_corr-=(best_parameters[1][0] ** 2 / (( X - best_parameters[1][1] ) ** 2 + best_parameters[1][0] ** 2) * best_parameters[1][2])
    y_bg_peak_corr-=(best_parameters[2][0] ** 2 / (( X - best_parameters[2][1] ) ** 2 + best_parameters[2][0] ** 2) * best_parameters[2][2])
...
def main():   
    #########################################################################
    ############################# LOADING DATA ##############################    
    a = numpy.loadtxt("semiamorphSi1-514-20s-02.txt")
    x = a[:,0]
    y = a[:,1]         
    #########################################################################
    ...
    # initial values #
    coefficients = [[3.5,521.6,12e3],[60,480,5000],[32,500,8000]]  # [hwhm, peak center, intensity] for three peaks
    #print (coefficients)
    # optimization # 
    pbest = leastsq(residuals, coefficients, args=(y_bg_corr,x), full_output=1)
    best_parameters = [pbest[0][:3],pbest[0][3:6],pbest[0][6:]]
